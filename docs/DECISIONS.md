...
- **Consequences**:
  ...
  - **Applied to wallet reconciliation (Sprint 6 slice 6).** `getLastReconciliationRun` + `GET /api/v1/admin/wallet-reconciliation/last-run` use the identical shape: `queue.getJobs(["completed"])` → filter by job name → pick by `finishedOn` → return `returnvalue` as the typed summary. The wallet reconciliation worker was updated to *return* its summary (previously `void`) so BullMQ now stores it on `Job.returnvalue`. The existing `POST /run` route stays inline since the operator-initiated run is synchronous-by-design (operator wants the result in the response, not a job id to poll); only the *scheduled* run benefits from the queue-introspection view. No DB schema change, no new audit shape, 7 unit tests cover the same edge cases as the platform-monitor instance (empty queue, foreign jobs, still-running jobs, freshest-by-finishedOn, missing returnvalue, queue-throws, ignoring newer foreign jobs).
  - **Applied to domain health (Sprint 6 slice 7) — and the migration that enabled it.** The Sprint 5 slice 1 domain-health worker was originally a `setInterval` started from `index.ts`. That's correct for single-instance dev but wrong for the production deploy (Render scale-out, k8s): `setInterval` fires once per process, so under N replicas the scan fires N times every cadence, producing N times the writes and N times the LLM/registrar load. The slice does two things in one move: (a) migrate the worker to BullMQ — a new `DOMAIN_HEALTH` queue + `DomainHealthJobData` type, a `Worker` that returns the typed `{scanned, escalated}` summary, and a `upsertJobScheduler` that holds the 6-hour cadence in Redis so all replicas share one tick; (b) add `getLastDomainHealthScan()` + `GET /api/v1/partner/domains/health/last-run` for free off BullMQ's `Job.returnvalue`. Partner UI surfaces "Last scheduled scan: 4h ago · 32 domains scanned" under the page header, so a partner can distinguish "no drift today because all green" from "no drift today because the scheduler hasn't run since redeploy." 7 unit tests mirror the platform-monitor/wallet-reconciliation set. This is the first `setInterval → BullMQ` migration of three; customer health and retention scan are the same shape but are deferred until they're each demonstrably needed on a multi-instance deploy.
